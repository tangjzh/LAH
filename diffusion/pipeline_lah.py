...
from datasets.camera_utils import generate_poses
...
class LattePipeline(DiffusionPipeline):
    ...
    def prepare_cameras(self, batch_size, video_length, anchor, device, poses=None, self_centric=True):
        shape = (batch_size, video_length, 12)

        if poses is None:
            if self_centric:
                poses = generate_poses([-30, 0, 30], list(range(0, 360, 60)), anchor)
                poses = torch.tensor([poses for _ in range(batch_size)]).to(device)
            else:
                raise NotImplementedError("Continous camera trajectory is not supported now.")
        else:
            if self_centric:
                gt_poses = generate_poses([-30, 0, 30], list(range(0, 360, 60)), anchor)
                poses = torch.tensor([gt_poses[i] if pose is None else pose 
                                      for i, pose in enumerate(poses)]).to(device)
            else:
                raise NotImplementedError("Continous camera trajectory is not supported now.")
    ...
    def __call__(
        self,
        camera_poses: Union[np.ndarray, List[np.ndarray]] = None,
        ...
        camera_poses = self.prepare_cameras(
            batch_size,
            video_length,
            0,
            device,
