...
def main(args):
    ...
    if args.enable_vae_temporal_decoder:
        vae = AutoencoderKLTemporalDecoder.from_pretrained(args.vae, torch_dtype=torch.float16).to(device)
    else:
        vae = AutoencoderKL.from_pretrained(args.vae, torch_dtype=torch.float16).to(device)
    tokenizer = T5Tokenizer.from_pretrained(args.text_encoder)
    text_encoder = T5EncoderModel.from_pretrained(args.text_encoder, torch_dtype=torch.float16).to(device)
